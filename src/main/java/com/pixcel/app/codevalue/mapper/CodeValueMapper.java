...
public interface CodeValueMapper {
    ...
    // 수정 시 자기 자신 제외 코드값 이름 중복 확인
    int countDuplicateSettingNameForUpdate(
            @Param("settingCodeId") Integer settingCodeId,
            @Param("userId") String userId,
            @Param("settingGroupName") String settingGroupName,
            @Param("settingName") String settingName
    );
    ...
    // 특정 코드값이 기본값인지 확인
    int countDefaultCodeValueById(@Param("settingCodeId") Integer settingCodeId);
    ...
    // 특정 코드값을 기본값으로 설정
    int updateDefaultYnToY(@Param("settingCodeId") Integer settingCodeId);
    ...
    List<CodeValueVO> selectReplacementCodeValueList(
            @Param("userId") String userId,
            @Param("settingGroupName") String settingGroupName,
            @Param("deleteSettingCodeId") Integer deleteSettingCodeId
    );

    // 삭제 대상 코드값을 실제 데이터가 사용 중인지 확인
    int countUsedCodeValue(@Param("settingCodeId") Integer settingCodeId);

    // 삭제 대상 코드값을 참조 중인 데이터를 대체 코드값으로 변경
    int updateUsedCodeValueToReplacement(
            @Param("deleteSettingCodeId") Integer deleteSettingCodeId,
            @Param("replaceSettingCodeId") Integer replaceSettingCodeId
    );
}